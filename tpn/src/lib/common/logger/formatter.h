...
template <>
struct fmt::formatter<tpn::log::LogMsg> : fmt::formatter<std::string_view> {
  template <typename FormatContext>
  auto format(tpn::log::LogMsg msg, FormatContext &ctx) {
    return format_to(ctx.out(), "{} : {}", msg.logger_name, msg.content);
  }
};
