...
bool DataHubMgr::Load(std::string_view path, std::string &error, bool reload /*  = false */) {
  if (!reload) {
...
bool DataHubMgr::Reload(std::string &error) {
  return Load({}, error, true);
}

std::string_view DataHubMgr::GetPath() {
  return path_;
}

/// uint32_t-id
const DataHubEntryItem::Item *DataHubMgr::GetDataHubEntryItem(uint32_t id) const {
...
/// uint32_t-level
const DataHubEntryLevel::Level *DataHubMgr::GetDataHubEntryLevel(uint32_t level) const {
...
/// uint32_t-id
const DataHubEntryPack::Pack *DataHubMgr::GetDataHubEntryPack(uint32_t id) const {
  std::string map_key = "" + ToString(id);
...
/// uint32_t-id
const DataHubEntryShop::Shop *DataHubMgr::GetDataHubEntryShop(uint32_t id) const {
  std::string map_key = "" + ToString(id);
...
const DataHubEntrySkill::Skill *DataHubMgr::GetDataHubEntrySkill(uint32_t id, uint32_t level) const {
  ...
  for (auto &&[key, val] : data_map.datas()) {
    if (val.Is<DataHubEntryItem>()) {
      ...
      for (auto &&data : item.datas()) {
        ...
        TPN_ASSERT(0 == item_map_.count(map_key), "map key repeated. {}", map_key);
      ...
      for (auto &&data : level.datas()) {
        ...
        TPN_ASSERT(0 == level_map_.count(map_key), "map key repeated. {}", map_key);
      ...
      for (auto &&data : pack.datas()) {
        std::string map_key = "" + ToString(data.id());
        TPN_ASSERT(0 == pack_map_.count(map_key), "map key repeated. {}", map_key);
      ...
      for (auto &&data : shop.datas()) {
        std::string map_key = "" + ToString(data.id());
        TPN_ASSERT(0 == shop_map_.count(map_key), "map key repeated. {}", map_key);
      ...
      for (auto &&data : skill.datas()) {
        ...
        TPN_ASSERT(0 == skill_map_.count(map_key), "map key repeated. {}", map_key);
